...
\begin{figure}[h]
	\begin{tikzpicture}
		\begin{groupplot}[height=25cm,group style={group size= 1 by 2}]
			\nextgroupplot[
				title={Storage Space/Query Speed},
				width=8cm,
				height=8cm,
				xlabel={Space [\% of input size]},
				ylabel={Time for $10^6$ queries. [$\lg 2$ ms]},
				legend pos=north east,
				%plotticks,
				%xtick=\empty,
				%ybar,
			...
			%% MULTIPLOT(ds,arity,filetype) SELECT AVG(space * 100.0 / input_size) AS x, LOG(2, query_time_total) AS y, MULTIPLOT
			%% FROM data WHERE NOT ds="string" AND NOT input_file LIKE "%200" AND type="random_access" AND (prefix_suffix=0 OR ds!="blocktree") AND (input_file="fib41" OR input_file LIKE "einstein%")
			%% GROUP BY ds,filetype,arity ORDER BY (x / y)
			\addplot coordinates { (0.199655,8.57743) };
			\addlegendentry{A2PS*,filetype=bt};
			\addplot coordinates { (0.328784,12.9121) };
			\addlegendentry{LzEnd};
			\addplot coordinates { (1.25271,6.56986) };
			\addlegendentry{A32PS*,filetype=bt};
			\addplot coordinates { (2.40126,8.99435) };
			\addlegendentry{Sampled Scan (Sequitur)};
			\addplot coordinates { (2.40209,8.93958) };
			\addlegendentry{Sampled Scan (RePair)};
			...
			%% MULTIPLOT(ds,arity,prefix_suffix) SELECT AVG(space * 100.0 / input_size) AS x, LOG(2, query_time_total) AS y, MULTIPLOT
			%% FROM data WHERE NOT ds="string" AND NOT input_file LIKE "%200" AND type="random_access" AND ds="blocktree" AND (input_file="fib41" OR input_file LIKE "einstein%")
			%% GROUP BY ds,arity,prefix_suffix ORDER BY (x / y)
			\addplot coordinates { (0.199655,8.57743) };
			\addlegendentry{A2PS0};
			\addplot coordinates { (0.684818,8.56605) };
			\addlegendentry{A2PS4};
			\addplot coordinates { (1.25271,6.56986) };
			\addlegendentry{A32PS0};
			\addplot coordinates { (1.71877,8.56605) };
			\addlegendentry{A2PS16};
			\addplot coordinates { (1.75177,6.56986) };
			\addlegendentry{A32PS4};
			\addplot coordinates { (3.24872,6.61471) };
			\addlegendentry{A32PS16};
...
\begin{figure}[h]
	\begin{tikzpicture}
		\begin{groupplot}[height=25cm,group style={group size= 1 by 2}]
			...
			%% AND (arity=2 AND prefix_suffix=0 OR arity=32 AND prefix_suffix=16 or ds!='blocktree')  
			%% GROUP BY ds,filetype,arity ORDER BY ds,filetype
			\addplot coordinates { (3.87678,11.2825) };
			\addlegendentry{A2PS*};
			\addplot coordinates { (17.7859,6.93074) };
			\addlegendentry{A32PS*};
			\addplot coordinates { (5.2398,15.0138) };
			\addlegendentry{LzEnd};
			\addplot coordinates { (3.63216,9.59991) };
			\addlegendentry{Sampled Scan (RePair)};
			\addplot coordinates { (3.27067,10.021) };
			\addlegendentry{Sampled Scan (Sequitur)};
			...
			%% AND (arity=2 AND prefix_suffix=0 OR arity=32 AND prefix_suffix=16 or ds!='blocktree')  
			%% GROUP BY ds,filetype,arity ORDER BY ds,filetype
			\addplot coordinates { (3.87678,21.0716) };
			\addplot coordinates { (17.7859,15.7193) };
			\addplot coordinates { (5.2398,23.6986) };
			\addplot coordinates { (3.63216,17.5317) };
			\addplot coordinates { (3.27067,17.7543) };
